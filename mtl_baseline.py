...
def validation_iters(validation_loader):
    ...
    for idx, data in enumerate(validation_loader):
        ...
        main_loss = criterion(scaled_forecast.squeeze(), simple_target_main)
...
for epoch in range(10000):
    ...
    for num_batches in range(500):
        ...
        for i in range(len(train_datasets_that_can_be_used)): 
            ...
            if random_dataset_chosen == args.validation_dataset_name:
                losses.append(loss*len(train_datasets_that_can_be_used))
            else:
                losses.append(loss)
