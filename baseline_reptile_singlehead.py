...
class NLinear(nn.Module):
    ...
    def forward(self, simple_input, dataset_name): 
        ...
        basis_parameters = self.basis_parameters_dict[args.validation_dataset_name](block_input)    #HERE ONLY USE THE VALIDATION DATASET NAME
...
for epoch in range(1000):
    ...
    for num_batches in range(500):
        ...
        if simple_input.shape[1]<input_range_len:
            simple_input = torch.cat((torch.zeros((simple_input.shape[0], input_range_len-simple_input.shape[1])).float().to(device), simple_input), axis=1)
            # print("simple_input.shape[1]<<<<<<<<input_range_len")
        else:
            simple_input = simple_input[:,-input_range_len:]
            # print("simple_input.shape[1]>>>>>>>input_range_len")
        ...
        if simple_target.shape[1]>len_forecast_horizons:
            simple_target = simple_target[:, :len_forecast_horizons]
            loss = criterion(scaled_forecast, simple_target)
            # print("simple_target.shape[1]>>>>>>>>>>len_forecast_horizons")

        else:
            loss = criterion(scaled_forecast[:,:simple_target.shape[1]], simple_target)
            # print("simple_target.shape[1]<<<<<<<<<<<<len_forecast_horizons")
